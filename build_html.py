...
function initSplash() {{
  ...
  if (splashReviewInfo && lesson.title.includes(' / ')) {{
    ...
    splashTitleEl.innerHTML = splashParts[0] + '<br><span style="opacity:0.75;font-size:0.88em">' + splashParts[1] + '</span>';
  }} else {{
...
function renderCard() {{
  ...
  // Build title HTML — split review lessons onto two lines
  let titleHtml;
  if (isReview && l.title.includes(' / ')) {{
    const parts = l.title.split(' / ');
    titleHtml = parts[0] + '<br><span style="opacity:0.7;font-size:0.85em">' + parts[1] + '</span>';
  }} else {{
